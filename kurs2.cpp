...
public:
	...
	Vector<Move> getMoves(char color)
	{
		Vector<Move> moves;
		bool jump = false;
		if (color == 'w')
	...
	Vector<Move> getMovesJump(char color, int x, int y)
	{
		Vector<Move> moves;
...
void printBoard()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            {
                std::cout << BLUE_COLOR << "@@" << RESET_COLOR;
            }
            else if (board[j][i] == ' ')
            {
                std::cout << "  ";
            ...
            {
                std::cout << WHITE_COLOR << "##" << RESET_COLOR;
            }
        }
        std::cout << "\n";
    }
...
int main()
{
	...
	while (game.getMoves('w')[0].x1 != -1 && game.getMoves('b')[0].x1 != -1)
	{
		...
		int max = ai.miniMax(game.getNewBoard(moves[0]), 10, 'b');
		for (auto move : moves)
		{
			if (ai.miniMax(game.getNewBoard(move), 10, 'b') > max)
			{
				max = ai.miniMax(game.getNewBoard(move), 10, 'b');
				maxMove = move;
